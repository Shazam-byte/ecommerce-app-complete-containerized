...
async function seedInitialData() {
    ...
    const checkCategory = await (0, connection_1.query)("SELECT COUNT(*) as count FROM categories");
    ...
    if (categoryCount === 0) {
        ...
        // Seed Categories
        await (0, connection_1.query)("INSERT INTO categories (name, slug) VALUES ($1, $2)", ["Electronics", "electronics"]);
        await (0, connection_1.query)("INSERT INTO categories (name, slug) VALUES ($1, $2)", ["Apparel", "apparel"]);
        await (0, connection_1.query)("INSERT INTO categories (name, slug) VALUES ($1, $2)", ["Books", "books"]);
        await (0, connection_1.query)("INSERT INTO categories (name, slug) VALUES ($1, $2)", ["Home & Kitchen", "home-kitchen"]);
        // Fetch newly created categories to match IDs
        const categoriesRows = await (0, connection_1.query)("SELECT id, name FROM categories");
        const categoriesMap = {};
        ...
        for (const p of productsToSeed) {
            await (0, connection_1.query)(`INSERT INTO products (category_id, name, slug, description, price, stock, images) 
         VALUES ($1, $2, $3, $4, $5, $6, $7)`, [p.category_id, p.name, p.slug, p.description, p.price, p.stock, p.images]);
        }
    }
    // Check if users are seeded
    const checkUsers = await (0, connection_1.query)("SELECT COUNT(*) as count FROM users");
    ...
    if (usersCount === 0) {
        ...
        // Create Admin
        await (0, connection_1.query)("INSERT INTO users (email, password_hash, role) VALUES ($1, $2, $3)", ["admin@catalog.com", adminPasswordHash, "admin"]);
        // Create Normal User
        await (0, connection_1.query)("INSERT INTO users (email, password_hash, role) VALUES ($1, $2, $3)", ["user@catalog.com", userPasswordHash, "user"]);
